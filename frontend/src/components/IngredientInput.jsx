        ...
        {recipe && (
          <div className="mt-8 rounded-3xl border border-[#E8E2D7] bg-[#FCF8F2] p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#2E7D32]">Your recipe</p>
                <h3 className="mt-2 text-2xl font-bold text-[#143D2A]">{recipe.title}</h3>
              </div>
            ...
            <p className="mt-4 leading-7 text-[#5F6F65]">{recipe.description}</p>

            <div className="mt-6 grid gap-6 md:grid-cols-2">
              <div>
                <h4 className="text-lg font-semibold text-[#143D2A]">Ingredients</h4>
                <ul className="mt-3 space-y-2 text-[#5F6F65]">
                  {recipe.ingredients?.map((item, index) => (
                    <li key={`${item}-${index}`} className="flex items-start gap-2">
                      <span className="mt-1 h-2 w-2 rounded-full bg-[#2E7D32]" />
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
              ...
              <div>
                <h4 className="text-lg font-semibold text-[#143D2A]">Steps</h4>
                <ol className="mt-3 space-y-2 text-[#5F6F65]">
                  {recipe.steps?.map((step, index) => (
                    <li key={`${step}-${index}`} className="flex gap-2">
                      <span className="font-semibold text-[#2E7D32]">{index + 1}.</span>
                      <span>{step}</span>
                    </li>
                  ))}
                </ol>
            ...
            <div className="mt-6 rounded-2xl border border-[#E8E2D7] bg-white p-4">
              <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#2E7D32]">Quick swaps</p>
              <ul className="mt-3 space-y-2 text-sm text-[#5F6F65]">
